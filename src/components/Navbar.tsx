...
const Navbar: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (token) {
      dispatch(login(token)); // Mark the user as logged in
    } else {
      dispatch(logout()); // If no token, mark the user as logged out
    }
  }, [dispatch]);
  ...
  return (
    <nav className="bg-white p-3 border-b shadow-sm sticky top-0 z-50">
      ...
      {/* Mobile Menu */}
      {/* {isMobileMenuOpen && (
        <div className="md:hidden bg-red-50 mt-2 shadow-md rounded">
          <Link href="/" className="block px-4 py-3 hover:text-red-500">Home</Link>
          <Link href="/shop" className="block px-4 py-3 hover:text-red-500">Shop</Link>
          <Link href="/about" className="block px-4 py-3 hover:text-red-500">About Us</Link>
          <Link href="/wishlist" className="block px-4 py-3 hover:text-red-500">Wishlist</Link>
        </div>
      )} */}
      {isMobileMenuOpen && (
